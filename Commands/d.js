...
module.exports = function (message) {
    ...
    if (words[1] == "a" || words[1] == "attack") {
        if (ts - cds[1] < 5000) { return functions.replyMessage(message, "On cooldown. Try again in " + functions.displayTime(cds[1], ts)); }
        cds[1] = ts;
        ...
        if (ts - cds[2] < 3000) { return functions.replyMessage(message, "On cooldown. Try again in " + functions.displayTime(cds[2], ts)); }
        cds[2] = ts;
        ...
        if (ts - cds[3] < 5000) { return functions.replyMessage(message, "On cooldown. Try again in " + functions.displayTime(cds[3], ts)); }
        cds[3] = ts;
        for (i = dam.length - 1; i >= 0; i--) {
            if (dam[i].action == 6) {
        ...
        if (ts - cds[4] < 4000) { return functions.replyMessage(message, "On cooldown. Try again in " + functions.displayTime(cds[4], ts)); }
        cds[4] = ts;
        ...
        if (ts - cds[5] < 5000) { return functions.replyMessage(message, "On cooldown. Try again in " + functions.displayTime(cds[5], ts)); }
        cds[5] = ts;
        for (i = dam.length - 1; i >= 0; i--) {
            if (dam[i].action == 4) {
                rnd = Math.floor(rnd * 1.3);
        ...
        if (ts - cds[6] < 4000) { return functions.replyMessage(message, "On cooldown. Try again in " + functions.displayTime(cds[6], ts)); }
        cds[6] = ts;
        ...
        if (ts - cds[7] < 20000) { return functions.replyMessage(message, "On cooldown. Try again in " + functions.displayTime(cds[7], ts)); }
        cds[7] = ts;
        cds[0] = true;
