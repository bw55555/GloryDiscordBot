...
module.exports = function (message) {
  ...
  if (words.length == 1) {
    ...
    if (userData[id].health > 10) { //debuffs stats if they are above a certain level
      userData[id].health -= 10;
    }
    if (userData[id].attack > 1) {
      userData[id].attack -= 1;
    }
    if (userData[id].defense > 1) {
  ...
  if (words.length == 2 && userData[id].triangleid == 5) {
    if (userData[id].cooldowns.rez > ts) {
