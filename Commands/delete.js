...
module.exports = async function (message, user) {
    ...
    if (admins.indexOf(user._id) == -1 && user.startts != undefined && functions.calcTime(user.startts + 24 * 60 * 60 * 1000, ts) > 0) { return functions.replyMessage(message, "You cannot delete your account yet! Please wait " + functions.displayTime(user.startts + 24 * 60 * 60 * 1000, ts))}
