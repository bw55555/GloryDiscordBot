...
function evaluateMessage(message) {
    ...
    functions.getUser(message.author.id).then(user => {
        ...
        if (user.maintenance) {
            ...
            functions.sendMessage(message.channel, ":arrow_up:  **MAJOR GLORY UPDATES**  :arrow_up:\n\n:earth_americas:   **No More Channels!**\n- Raids are no longer bound to channels. \n- Do `!travel <loc>` to journey to distant lands. Proceed to battle mythical creatures with `!ratk`.\n- Join comrades by adding the *raid ID* after the location name. \n- Do `!travel list` and start your expedition! More areas will be discovered and added to the list in the future as we continue to venture into the unknown.\n- Travel will have a cooldown of 30 seconds. \n\n:crossed_swords:   **Raid Adjustments**\n- Fixed *Asmodeus* and *Medusa*. \n- Added status Effects: \n    - **Silence** `(No raids yet)`: no enchantments will work. This includes guild buffs, weapon enchantments, everything. \n    - **Stun** `(No raids yet)`: You cannot attack.\n    - **Petrify** `(Medusa Raid)`: Applies both silence and stun. (What can you do if you're turned to stone anyway?)\n    - **Vulnerable** `(No raids yet)`: Stacks on your character. If you have *x stacks of vulnerable*, you have *2x% less BASE defense*.\n    - **Weakness** `(No raids yet)`: Stacks on your character. If you have *x stacks of weakness*, you have *2x% less BASE attack*. \n    - **Bleed** `(Asmodeus Raid)`: Stacks on your character. If you have *x stacks of bleed*, you have *x stacks of both vulnerable and weakness*. If you have *50 stacks of bleed*, you die.\n\n:desktop: **Changed Commands**\n- Deleted commands: `!watk`, `!eatk`, `!g ratk` (replaced these with `!ratk` after `!travel`ling to their respective location)\n- Deleted `!summon` (replaced with `travel`ling to an instance) \n- Added `!location` (see your current location)\n- Added `!status` (see your status effects - currently burn, bleed, bolster)\n\n:tada: :gift:  **FEELING LUCKY!? **  :gift: :tada:\n\nIt's our glorious dev's birthday on the **11th of November**. To celebrate, everyone will have `2x lucky` from **Nov 11 UTC 0:00** to **Nov 11 UTC 23:59**.\n\nLet's continue to seek **GLORY** together.")
        }
