...
function locationsummon(raid) {
    if (raid.elitesummon == undefined) { raid.elitesummon = 0 }
    ...
    } else if (rng < 0.2 + raid.elitesummon + raid.areabosssummon) {
        rnum = 2;
        raid.elitesummon = 0
        raid.areabosssummon = 0
    } else {
        rnum = 0;
        raid.elitesummon += 0.1
...
function checkProps(message,user) {
    ...
    for (let k in templateUser) {
        ...
        if (typeof templateUser[k] == "number" && isNaN(user[k])) {
            user[k] = templateUser[k]
            errorlog({ stack: "attribute " + k + " is not a number when it should be for user " + user._id })
            user.flag = true
        }
    }
    ...
    */
    ...
    if (raid.currenthealth <= 0) {
        ...
        if (type == "event" || type == "world") {
            ...
            let totalglory = (raid.health / 100000)
